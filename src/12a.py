...
def get_perimeter(region) -> int:
    perimeter_map = {}
    for x, y in region:
        up = tuple(sorted([(x, y), (x, y-1)]))
        down = tuple(sorted([(x, y), (x, y+1)]))
        left = tuple(sorted([(x, y), (x-1, y)]))
        right = tuple(sorted([(x, y), (x+1, y)]))
        for surrounding in [up, down, left, right]:
            if surrounding not in perimeter_map:
                perimeter_map[surrounding] = []
            perimeter_map[surrounding].append((x, y))
    perimeter = 0
    for vv in perimeter_map.values():
        if len(vv) == 1:
            perimeter += 1
    return perimeter
