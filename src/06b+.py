...
def find_obstacle_ahead(cur_pos, facing, obstacles) -> tuple:
...
def get_ans(file_path: str):
    ...
    while True:
        ...
        if (
            next_pos not in loop_points and
            next_pos not in visited and
            traverse_map[next_pos[1]][next_pos[0]] != '#'
        ):
            obstacles.add(next_pos)
            brancing_idx = len(lines)
            lines.append((last_turn_pos, cur_pos))
            if is_looping(cur_pos, (-facing[1], facing[0]), lines, obstacles):
                loop_points.add(next_pos)
            obstacles.remove(next_pos)
            lines = lines[:brancing_idx]
